...
if [ $# -eq 0 ] ; then
  ...
  curl -O "https://s3-us-west-2.amazonaws.com/sourmash-databases/genbank-d2-k31.tar.gz"
  tar xzf genbank-d2-k31.tar.gz
  echo '[' > .dotfiles
  ...
  echo '{"name":".sbt.genbank-d2-k31", "type":"directory","mtime":"1","size":"1"}' >> .dotfiles
  echo ']' >> .dotfiles
  # if [[ -d ".sbt.genbank-d2-k31" && -d ".sbt.img_arch_isol" && -d ".sbt.img_bact_mags" ]] ; then
  if [[ -d ".sbt.genbank-d2-k31" ]] ; then
    touch __READY__
  else
    echo "init failed"
  fi
